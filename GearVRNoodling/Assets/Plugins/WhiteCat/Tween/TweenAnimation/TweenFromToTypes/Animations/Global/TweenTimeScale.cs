...
namespace WhiteCat
{
	...
	public class TweenTimeScale : TweenFloat
	{
		...
		{
			if (tweener && tweener.timeMode == TimeMode.Normal)
			{
				EditorGUILayout.HelpBox("Use Unscaled time mode for Tweener to avoid unexpected result.", MessageType.Warning, true);
