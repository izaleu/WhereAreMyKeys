...
namespace WhiteCat
{
	...
	public partial class MoveAlongPath
	{
		...
		static GUIStyle _tableHeaderLabel;
		...
		protected override void Editor_OnInspectorGUI()
		{
			...
			if (_path)
			{
				...
				{
					_tableHeaderLabel = new GUIStyle(EditorStyles.boldLabel);
					_tableHeaderLabel.alignment = TextAnchor.MiddleCenter;
				}
				...
				if (_keyframeLists.Count > 0)
				{
					...
					EditorGUI.LabelField(left, "Keyframe List", _tableHeaderLabel);
					EditorGUI.LabelField(right, "Target Component", _tableHeaderLabel);
					...
					Path.KeyframeListTargetComponentPair pair;
