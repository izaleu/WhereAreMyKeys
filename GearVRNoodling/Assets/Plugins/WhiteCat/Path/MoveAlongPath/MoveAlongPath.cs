...
namespace WhiteCat
{
	...
	public partial class MoveAlongPath : ScriptableComponentWithEditor
	{
		...
		[SerializeField] Path.Location _location = new Path.Location();

		[SerializeField] float _speed = 1f;
		[SerializeField] TimeMode _timeMode = TimeMode.Normal;

		[SerializeField] List<Path.KeyframeListTargetComponentPair> _pairs = new List<Path.KeyframeListTargetComponentPair>(4);
		...
		public bool AddMovingObject(Path.KeyframeList keyframeList, Component target)
		{
			...
			{
				var movingObject = new Path.KeyframeListTargetComponentPair();
