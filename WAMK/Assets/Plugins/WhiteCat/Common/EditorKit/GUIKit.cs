	...
	public partial struct EditorKit
	{
		...
		public static Color randomColor
		{
			get
			{
				...
				values[0] = Kit.random.Range(0f, 0.6f);
				values[1] = Kit.random.Range(0.4f, 1f);

				float min = Mathf.Min(values[0], values[1]);
				float max = Mathf.Max(values[0], values[1]);

				if (max - min > 0.4f)
				{
					values[2] = Kit.random.Range(0f, 1f);
				}
				else
				{
					float t = Kit.random.Range(0f, max - min + 0.2f);
					if (t < max - 0.4f) values[2] = t;
					else values[2] = min + t - max + 0.8f;
				}

				int b = Kit.random.Range(0, 3);
				return new Color(values[(b + 1) % 3], values[(b + 2) % 3], values[b]);
			}
		}
