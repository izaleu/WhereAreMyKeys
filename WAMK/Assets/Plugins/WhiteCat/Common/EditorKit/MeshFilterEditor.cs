...
namespace WhiteCatEditor
{
	...
	public class MeshFilterEditor : BaseEditor
	{
		...
		void DrawNormals()
		{
			EditorKit.RecordAndSetHandlesMatrix(transform.localToWorldMatrix);
			...
			if (!Kit.IsNullOrEmpty(normals))
			{
				...
				EditorKit.RecordAndSetHandlesColor(normalsColor);
				for (int i = 0; i < count; i++)
				{
					EditorKit.HandlesDrawAALine(vertices[i], vertices[i] + normals[i] * normalsSize);
				}
		...
		void DrawVertices()
		{
			...
			Handles.BeginGUI();
			EditorKit.RecordAndSetGUIColor(verticesColor);
		...
		void DrawBounds()
		{
			...
			EditorKit.RecordAndSetHandlesColor(boundsColor);
			EditorKit.HandlesDrawWireBounds(mesh.bounds);
			EditorKit.RestoreHandlesColor();
