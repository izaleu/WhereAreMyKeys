...
namespace WhiteCat
{
	...
	{
		// 材质类型
		[SerializeField] MaterialType _materialType = MaterialType.Shared;
		...
		public void UpdateMaterial()
		{
			...
			if (_materialType == MaterialType.Shared)
			{
		...
		{
			float lines = _materialType == MaterialType.Shared ? 4f : 5f;
		...
		protected override void Editor_OnGUI(Rect rect, SerializedProperty property, GUIContent label)
		{
			...
			if (_materialType == MaterialType.Shared)
			{
				...
				EditorGUI.BeginChangeCheck();
				var newSharedMaterial = EditorGUI.ObjectField(rect, "Shared Material", _sharedMaterial, typeof(Material), false);
				if (EditorGUI.EndChangeCheck())
				{
					Undo.RecordObject(target, "Shared Material");
					sharedMaterial = newSharedMaterial as Material;
					EditorUtility.SetDirty(target);
				}
			}
			else
